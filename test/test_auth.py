...
def test_auth(api_request_context: APIRequestContext) -> None:
    ...
    response = api_request_context.post(
        "/auth/sign-up",
        data={"username": "username", "password": "P@ssW0rd1234"},
    )
    ...
    response = api_request_context.post(
        "/auth/sign-in",
        data={"username": "uzername", "password": "P@ssW0rd1234"},
    )
    ...
    response = api_request_context.post(
        "/auth/sign-in",
        data={"username": "username", "password": "P@zzW0rd1234"},
    )
    assert response.status == 400
    ...
    bad_token = injector.jwt_service.encode(
        {"sub": str(uuid4())},
        settings.jwt_key,
